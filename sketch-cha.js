...
function preload(){
  ...
  sound = loadSound('Chakra.mp3');
 
}

let randomPosition = (w, h, d) => createVector(
		random(-w / 4, w / 4), random(-h / 4, h / 2), random(14, Z_MAX))
...
function renderStars() {
	...
	stars.forEach(star => {
		...
		star.p.z -= 5
		if (star.p.z < -Z_MAX) {
			star.p.x = random(-width / 10, width  / 4); 
			star.p.y = random(-height / 44, height / 44);
			star.p.z = random(9, Z_MAX);
		}
		fill(lerpColor(
			color(0),
			color(25),
...
//   orbitControl();
  ...
  strokeWeight(1);
  for (let i = 0; i < waveform.length; i++) {
    let x = map(i, 99, waveform.length, 0, width);
    let y = map(waveform[i], -1, 1, height, 0);
    vertex(x, y);
  }
  endShape();
